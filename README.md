...
A pasta `model_evaluation` possui 2 notebooks. O primeiro é o arquivo `03_definicao_mvp.ipynb` onde foram testados diversos algoritmos de classificação para definir quais seriam os melhores candidatos para o problema em questão. Por fim o último notebook é o `04_model_performance.ipynb`, onde fiz a análise do modelo final, porém com uma visão voltada ao negócio. 

A próxima pasta é a `src` onde estão os scripts em python que utilizei para desenvolver o modelo de crédito. A pasta `feature_engineering` possui 6 arquivos script.py, sendo 4 scripts transformers criados para montar um pipeline que foi utilizado no arquivo `feat_eng_pipeline.py`. O pipeline foi criado com o intuito de simular um pipeline de dados em produção e foi utilizado no arquivo `feature_engineering.py`, onde realizei a filtragem e tratamento do dataset bruto, criei novas variáveis e por fim separei a base de treino e teste. 
A última dentro de `src` é a `model`. Essa pasta possui o script `training_model.py` onde defini um algoritmo para treinar com os dados de treino e avaliamos com o treino e teste. A resposta desse script está salva nas pastas `models` e `outputs`.
...
A EDA é iniciada no arquivo `01_eda.ipynb` na pasta `notebooks/eda`. Para termos uma EDA clara foram feitas as seguintes etapas
...
* Menos da metade das features poderá ser utilizada devido a uma alta frequência de nulos. Fiz uma filtragem das colunas com até 20% de valores nulos. Esse valor foi escolhido, pois a partir dai, os valores imputados para cobrir os nulos podem começar a atrapalhar o treinamento do modelo;
...
* Consegui identificar variáveis com um bom ordenamento para o target proposto. Como forma de conseguir features estáveis e identificar quais as melhores variáveis preditoras, irei realizar uma análise de Weight of Evidence, que irá complementar a EDA e já servir como Feature Engineering.

### Feature Engineering

Nessa etapa optei por fazer uma análise de Peso de Evidência para as features selecionadas na EDA. Isso foi feito para identificar as variáveis que pudessem ser boas preditoras, criar novas features baseadas no WoE e também identificar quais variáveis são estáveis tanto na originação quanto na relação com o target. A análise pode ser encontrada no notebook `02_feature_eng.ipynb` na pasta `notebooks/feature_engineering`. 
Após a definição das estratégias a seguir com a base bruta, criei os seguintes transformers:
...
A análise das métricas do negócio estão no notebook `04_model_performance.ipynb` na pasta `notebooks/model_evaluation`. O notebook tem os seguintes passos:
...
Realizando esses passos é possível ter a seguinte conclusão sobre o projeto:
